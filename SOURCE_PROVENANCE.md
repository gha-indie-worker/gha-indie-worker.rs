...
- Executable workflow admission: `src/gha_workflow.rs` is pinned to Git blob `bb7e3042b217cf154f432694dbf8866226bcb123`; it sends the production fixed-profile planner through shared strict YAML admission, accepts only lowercase immutable commit IDs, enforces GitHub-compatible job identifier syntax, refuses unevaluated or out-of-policy trigger semantics, and rejects commands outside the selected profile's reviewed coverage before fixed-profile binding
...
- Standalone operator contract: `readme.md` is pinned to Git blob `c947cec02f207d743d8a454c3e85a1ff73fe6825`; it documents immutable profile checkout separately from mutable image-job branch and tag checkout, plus the strict YAML, trigger-admission, and fixed-profile command-coverage boundaries
